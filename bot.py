...
TOKEN = '5722827215:AAE33MP5o28ZfucHgX9RFpp8ZqMtWSu2tug'
...
def db_table_val(user_id: int, user_name: str, user_surname: str,
    ...
    connect.commit() ###фиксирует текущую транзакцию. Если не вызывать этот метод, то все, что сделано после последнего вызова
    # connect. commit() , не будет видно из других соединений с базой данных.
...
@bot.message_handler(commands=['start'])##декоратор ожидает сообщения-команды 
def start(message):
    ...
    markup.add(types.KeyboardButton('Марки, которые мы обслуживаем:'))
...
def bot_message(message):
    if message.text == 'Оставьте заявку мастеру🛠.':
        ...
        ##сообщение, которое пересылается
        ...
        bot.send_message(message.chat.id,
                         ...
                         reply_markup=markup)
###В переменные мы вносим имя пользователя, id, имя и фамилию в Telegram, а потом сохраняем в БД.
        ...
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)##ReplyKeyboardMarkup - объект, который создает клавиатуру.Параметр resize_keyboard=True уменьшает ее размер.
    ...
    elif message.text == 'Марки, которые мы обслуживаем:':
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        bot.send_message(message.chat.id,
